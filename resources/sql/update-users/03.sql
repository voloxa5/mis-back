--синхронизирую юзеров
merge into proger.users target
using (
...
when matched
    then
    update
    set target.NAME=lower(trim(source.user_name)),
        target.EMAIL=lower(trim(source.user_name)),
        target.IS_LOCKED=0,
        target.PASSWORD_CHANGE_REQUEST=0
when not matched
    then
    insert (target.id_kaskad, target.NAME, target.EMAIL, target.IS_LOCKED, target.PASSWORD_CHANGE_REQUEST,
            target.IS_ADMIN, target.PASSWORD)
    values (source.id_kaskad,
            lower(trim(source.user_name)),
            lower(trim(source.user_name)),
            0, 0, 0, '$2y$10$HwmlQz7wlGQ6zxiJ5Nnvuel79bM1AIAGoHQVFLZmxFV5feLRSHft.')
